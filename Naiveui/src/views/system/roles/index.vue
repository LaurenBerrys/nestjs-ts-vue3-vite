  ...
  <ComponentPage>
    ...
    >
      <template #tableButton>
        <n-button type="primary" size="small" @click="handleAdd">添加角色</n-button>
      </template>
    </NvapTable>
...
<script lang="ts" setup>
  ...
  const handleMenuAuth = async (record: Recordable) => {
    ...
    const totree = async (data) => {
      data.forEach((res) => {
        if (res.permissions) {
          const arr = isArray(res.permissions) ? res.permissions : res.permissions.split(',');
          res.permissions = arr.filter((item) => record.permissions.includes(item));
        }
        if (res.children) {
          totree(res.children);
        }
