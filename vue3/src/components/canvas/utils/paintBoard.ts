...
import { FreeDraw, FreeDrawStyle, freeDrawRender, Material } from './element/freeDraw';
...
export type HistoryState = Pick<
  PaintBoard,
  'currentLineColor' | 'currentLineWidth' | 'cleanWidth' | 'originTranslate' | 'layer' | 'version'
>;
...
export class PaintBoard {
        ...
        if (ele?.layer && showLayerIds.has(ele.layer)) {
          ...
          switch (ele?.type) {
            case CANVAS_ELE_TYPE.FREE_DRAW:
              ...
              break;
            case CANVAS_ELE_TYPE.ERASER:
              eraserRender(this.context, this.cleanCanvas.bind(this), ele as Eraser);
              break;
  ...
  cache() {
    ...
    const { currentLineColor, currentLineWidth, cleanWidth, originTranslate, layer, version } =
      this;
    const state = {
      currentLineColor,
      currentLineWidth,
  ...
  canvasMouseUp() {
    if (this.select.isCurrentChange && this.select.tempCache) {
      this.history.pushStack(this.history.getCurrentStack(), this.select.tempCache);
    }
