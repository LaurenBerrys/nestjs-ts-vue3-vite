...
import { ElementRect, ELEMENT_INSTANCE, MousePosition } from '../type';
import { CANVAS_ELE_TYPE, RESIZE_TYPE } from './constants';
import { CURSOR_TYPE, getResizeCursorType } from './cursor';
import { drawRect, getDistance, getPositionToLineDistance, isInsideRect } from './common';
import { FreeDraw, FreeDrawRect, moveFreeDraw, resizeFreeDraw } from './element/freeDraw';
...
export class SelectElement {
  ...
  moveSelectElement(position: MousePosition) {
    ...
    if (this.selectElementIndex !== -1) {
      if (this.resizeType !== RESIZE_TYPE.NULL) {
        ...
        const resizeType = getResizeType(movePos, this.getCurSelectElement().rect);
  ...
  clickSelectElement(position: MousePosition) {
    ...
    // 在有选择元素，判断当前的点击位置
    if (this.selectElementIndex !== -1) {
      const resizeType = getResizeType(resizeMousePos, this.getCurSelectElement().rect);
 ...
 * 绘制拖拽矩形
 */
export const drawResizeRect = (context: CanvasRenderingContext2D, rect: ElementRect) => {
